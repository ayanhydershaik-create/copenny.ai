...
class PersonalizationEngine:
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        if MONGODB_AVAILABLE:
            try:
                self.mongodb = get_firestore_service()
    ...
    def validate_csv(self, csv_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            from app.tools.enhanced_csv_tools import _detect_columns
    ...
    def process_user_csv(
        ...
        try:
            ...
            # Copy CSV to user directory with retry for OneDrive/Windows file locks
            import shutil
            import time
            max_retries = 5
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        return {
                            "success": False,
                            "error": f"Failed to save data after several attempts (file may be locked): {str(e)}"
                        }
                    time.sleep(0.3 * (attempt + 1))
            
            # Load and process data
            # Ensure fresh read by bypassing any potential OS/Pandas level cache if we were using it (we aren't, but good to be explicit in thoughts)
